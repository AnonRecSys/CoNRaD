...
from util.utils import next_batch_pairwise_CLCRec_cold_deb
import torch.nn.functional as F


# Following the source code process: https://github.com/weiyinwei/CLCRec
class CLCRec(BaseColdDebTrainer):
    def __init__(
        self,
        args,
        training_data,
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(
                next_batch_pairwise_CLCRec_cold_deb(
            ...
            with torch.no_grad():
                ...
                self.item_emb.data[self.data.cold_item_list] = now_cold_item_emb
                if epoch % self.args.eval_freq == 0:
                    # self.fast_evaluation_quiet(epoch, valid_type='all')
                    self.fast_evaluation_quiet(epoch, valid_type="cold")
            if epoch + 1 - self.bestPerformance[0] >= self.args.patience:
                break

        self.user_emb = self.best_user_emb
    ...
    def save(self):
        with torch.no_grad():
            ...
            self.best_item_emb.data[self.data.cold_item_list] = now_best_cold_item_emb
...
class CLCRec_Learner(nn.Module):
    def __init__(self, args, data, emb_size, device):
        ...
        self.device = device
        self.data = data
        # self.item_content = torch.tensor(self.data.item_content, dtype=torch.float32, requires_grad=False).to(device)
        self.item_content = torch.cat(
            [F.normalize(torch.from_numpy(x).cuda()) for x in data.item_content], dim=1
        )
    ...
    def get_all_embs(self):
        ...
        return (
            ...
            feature[self.data.cold_item_list],
        )
