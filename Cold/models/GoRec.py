...
from .BaseRecommenderColdDeb import BaseColdDebTrainer
from util.utils import next_batch_pairwise_cold_deb
...
class GoRec(BaseColdDebTrainer):
    def __init__(
        self,
        args,
        training_data,
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(
                next_batch_pairwise_cold_deb(self.data, self.batch_size)
            ):
                u_idx, i_idx, _ = batch
            ...
            with torch.no_grad():
                ...
                if self.args.cold_object == "item":
                    ...
                    now_item_emb = self.model(
                        self.model.cluster_cfmean.to(self.device),
                        self.model.item_content,
                        gen_size=self.data.item_num,
                    )
                else:
                    now_user_emb = self.model(
                        self.model.embedding_dict["user_emb"], self.model.user_content
                    )
        ...
        if self.args.save_emb:
            ...
            torch.save(
                self.model.cluster_label,
                f"./emb/{self.args.dataset}_cold_{self.args.cold_object}_{self.args.model}_cluster_label.pt",
            )
    ...
    def save(self):
        with torch.no_grad():
            if self.args.cold_object == "item":
                ...
                now_best_item_emb = self.model(
                    self.model.cluster_cfmean.to(self.device),
                    self.model.item_content,
                    gen_size=self.data.item_num,
                )
            else:
                now_best_user_emb = self.model(
                    self.model.embedding_dict["user_emb"], self.model.user_content
                )
...
class GoRec_Learner(nn.Module):
    def __init__(self, args, data, emb_size, device, decoder_layer=2):
        ...
        if self.args.cold_object == "item":
            self.item_content = torch.cat(
                [F.normalize(torch.from_numpy(x).cuda()) for x in data.item_content],
                dim=1,
            )
        else:
        ...
        self.encoder = Encoder(
            latent_dim=self.latent_size, z_size=self.z_size, si_dim=self.content_dim
        )
        self.decoder = Decoder(
            z_size=self.z_size,
            latent_dim=self.latent_size,
            layer=decoder_layer,
            si_dim=self.content_dim,
        )
    ...
    def _init_model(self):
        user_emb, item_emb = torch.load(
            f"../MMRec_embs/{self.args.dataset}/{self.args.backbone}.pt",
            map_location="cpu",
        )
        ...
        # embedding_dict['item_emb'] = compress_norms(embedding_dict['item_emb'])
    ...
    def clustering(self):
        ...
        clust_path = "..data/%s/cold_deb/gorec_clust_%d.pkl" % (
            self.args.dataset,
            self.args.pre_cluster_num,
        )
        if os.path.exists(clust_path):
            self.cluster_label = pickle.load(open(clust_path, "rb"))
        else:
            cluster_fit = KMeans(
                n_clusters=self.args.pre_cluster_num, random_state=0
            ).fit(self.item_content.cpu().detach().numpy())
            self.cluster_label = cluster_fit.labels_
            pickle.dump(self.cluster_label, open(clust_path, "wb"))

        print("clustered")
        cluster_cfmean = []
        for i in range(self.args.pre_cluster_num):
            cluster_cfmean.append(
                self.embedding_dict[self.args.cold_object + "_emb"][
                    np.where(self.cluster_label[: len(self.data.warm_item_list)] == i)
                ].mean(0)
            )
    ...
    def forward(self, warm, side_information, gen_size=10):
        if self.training:
            ...
            # encode
            mu, log_variances, mu_zgc, log_variances_zgc = self.encoder(
                warm, side_information
            )
            ...
            sample_from_normal = Variable(
                torch.randn(len(warm), self.z_size).to(self.device), requires_grad=True
            )
            sample_from_normal_zgc = Variable(
                torch.randn(len(warm), self.z_size).to(self.device), requires_grad=True
            )
            ...
            if warm is None:
                ...
                z = Variable(
                    torch.randn(gen_size, self.z_size).to(self.device),
                    requires_grad=False,
                )
                ...
                # sample from gaussian
                sample_from_normal = Variable(
                    torch.randn(len(warm), self.z_size).to(self.device),
                    requires_grad=True,
                )
...
class Encoder(nn.Module):
    def __init__(self, latent_dim, z_size, si_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(
                in_features=(z_size + si_dim), out_features=(latent_dim), bias=False
            ),
