...
from util.utils import next_batch_pairwise_cold_deb, bpr_loss, mse_loss
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(
                ...
                # gen_loss = self.args.alpha * mse_loss(pos_item_emb.detach(), gen_emb)
            ...
            with torch.no_grad():
                ...
                cold_item_gen_emb = model.generate_item_emb(self.data.cold_item_list)
                self.item_emb = torch.cat([self.item_emb, cold_item_gen_emb])
        ...
        with torch.no_grad():
            ...
            if self.args.cold_object == "item":
                now_cold_item_gen_emb = self.model.generate_item_emb(
                    self.data.cold_item_list
                )
                self.best_item_emb = torch.cat(
                    [self.best_item_emb, now_cold_item_gen_emb]
                )
            else:
...
class GAR_Learner(nn.Module):
    ...
    def get_training_embs_r(self, uid, iid, nid):
        u_emb = self.embedding_dict["user_emb"][uid]
        user_emb_r = self.mlps["user"](u_emb)
