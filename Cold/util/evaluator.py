...
def ranking_evaluation(origin, res, N):
    ...
    for i, n in enumerate(N):
        ...
        # print(type(hr), type(prec), type(recall), type(NDCG))
        # AUC = Measure.AUC(origin,res,rawRes)
        # measure.append('AUC:' + str(AUC) + '\n')
        measure.append("Top " + str(n) + "\n")
        measure += indicators
        # performance[i].append(hr)
        # performance[i].append(prec)
...
def get_inds_in_batch(inds, i, batch_size):
    return (
        inds[
            ~(
                (inds < i * batch_size).astype(int)
                + (inds >= (i + 1) * batch_size).astype(int)
            ).astype(bool)
        ]
        - i * batch_size
    )
