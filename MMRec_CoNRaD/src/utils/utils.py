...
def get_trainer_conrad():
    return getattr(importlib.import_module("common.trainer_conrad"), "Trainer")


def get_trainer_me():
    return getattr(importlib.import_module("common.trainer"), "TrainerMultEval")
...
def build_sim(context):
    context_norm = context.div(torch.norm(context, p=2, dim=-1, keepdim=True) + 1e-7)
...
def get_sparse_laplacian(edge_index, edge_weight, num_nodes, normalization="none"):
    ...
    if normalization == "sym":
        deg_inv_sqrt = torch.maximum(deg, torch.tensor(0.0)).pow_(-0.5)
...
def build_knn_normalized_graph(adj, topk, is_sparse, norm_type):
    ...
    if is_sparse:
        ...
        return torch.sparse_coo_tensor(edge_index, edge_weight, adj.shape).cuda()
    else:
        weighted_adjacency_matrix = (torch.zeros_like(adj)).scatter_(
            -1, knn_ind, knn_val
        )
