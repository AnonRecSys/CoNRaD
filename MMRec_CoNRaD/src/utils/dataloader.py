...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        if config["use_neg_sampling"]:
            if self.use_full_sampling:
                ...
                if config["neg_sample_multi"]:
                    self.n_negs = config["num_neg"]
                    self.sample_func = self._get_neg_sample_multi
                else:
    ...
    def _get_neg_sample_multi(self):
        cur_data = self.dataset[self.pr : self.pr + self.step]
        self.pr += self.step
        # to tensor
        user_tensor = (
        ...
        u_ids = cur_data[self.config["USER_ID_FIELD"]]
        # sampling negative items only in the dataset (train)
        neg_ids = self._sample_neg_ids_multi(u_ids).to(self.device)
        item_tensor = torch.cat([pos_item_tensor.unsqueeze(1), neg_ids], dim=1)

        user_tensor = user_tensor.unsqueeze(1).repeat(1, 1 + self.n_negs)
        pos_item_tensor = pos_item_tensor.unsqueeze(1).repeat(1, 1 + self.n_negs)
        # for neighborhood loss
        batch_tensor = torch.cat((user_tensor, item_tensor, pos_item_tensor))
    ...
    def _sample_neg_ids_multi(self, u_ids):
        neg_ids = []
        for u in u_ids:
            u_negs = []
            # random 1 item

            while len(u_negs) < self.n_negs:
                iid = self._random_multi()
                u_negs = [x for x in iid if x not in self.history_items_per_u[u]][
                    : self.n_negs
                ]
            neg_ids.append(torch.tensor(u_negs).type(torch.LongTensor))
        return torch.stack(neg_ids)
    ...
    def _random_multi(self):
        rd_id = random.sample(self.all_items, int(self.n_negs * 1.3))
        return rd_id
