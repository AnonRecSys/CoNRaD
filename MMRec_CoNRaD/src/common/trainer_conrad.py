...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model, mg=False):
        ...
        if "clip_grad_bool" not in config:
            self.clip_grad_bool = True
        else:
            self.clip_grad_bool = config["clip_grad_bool"]
        ...
        try:
            self.save_full_model = config["save_full_model"]
        except:
            ...
            [
                {
                    "params": [
                        ...
                        if n.startswith("embedding_dict.")
                    ],
                    "weight_decay": 0.0,
                },
                {
                    "params": [
                        p
                        for n, p in self.model.named_parameters()
                        if not n.startswith("embedding_dict.")
                    ],
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            # if batch_idx == 0:
            #    break
        mx = torch.cuda.max_memory_allocated() / 1073741824
        print("MAX", mx)
    ...
    def fit(
        ...
        t_max = int(steps_per_epoch * self.epochs)
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=t_max, eta_min=0.0
        )
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                try:
                    self.model.beta = self.model.inference_betas[0]
                except:
                    pass
                ...
                if update_flag:
                    ...
                    self.model.eval()
                    self.best_embs = self.model.forward_pred()
                    self.model.state_dict()
                    self.model.train()
        try:
            # eval: To ensure the test result is the best model under validation data, set self.eval_step == 1
            for beta_step, inference_beta in enumerate(self.model.inference_betas):
                self.model.beta = inference_beta
                valid_start_time = time()
                valid_score, valid_result = self._valid_epoch(valid_data)
                self.best_valid_score, self.cur_step, stop_flag, update_flag = (
                    early_stopping(
                        ...
                        self.best_valid_score,
                        beta_step,
                        max_step=len(self.model.inference_betas),
                        bigger=self.valid_metric_bigger,
                    )
                )
                valid_end_time = time()
                ...
                if update_flag:
                    ...
                    self.model.eval()
                    self.best_embs = self.model.forward_pred()
                    self.best_inference_beta = inference_beta
            ...
            self.best_inference_beta = self.model.inference_betas[0]
        except:
            self.best_inference_beta = None
        return (
            self.best_valid_score,
            self.best_valid_result,
            self.best_test_upon_valid,
            self.best_inference_beta,
        )
...
class TrainerMultEval(AbstractTrainer):
    r"""The basic Trainer for basic training and evaluation strategies in recommender systems. This class defines common
     functions for training and evaluation processes of most recommender system models, including fit(), evaluate(),
    and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        super(TrainerMultEval, self).__init__(config, model)
    ...
    def fit(
        ...
        Returns:
             (float, dict): best valid score and best valid result. If valid_data is None, it returns (-1, None)
        """
        test_data, test_data_id = test_data
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            self.model.pre_epoch_processing()
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # test
                _, test_result = self._valid_epoch(test_data)
                _, test_result_id = self._valid_epoch(test_data_id)
                if verbose:
                    self.logger.info(valid_score_output)
                    self.logger.info(valid_result_output)
                    self.logger.info("test result: \n" + dict2str(test_result))
                if update_flag:
                    ...
                    self.best_valid_result = valid_result
                    self.best_test_upon_valid = test_result
                    self.best_test_id_upon_valid = test_result_id
